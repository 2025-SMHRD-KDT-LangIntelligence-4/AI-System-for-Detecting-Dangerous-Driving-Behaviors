...
public class ExampleController2 {
	...
	// 뭘 연결해? ExampleService라는 서비스와 연결하겠다. 그리고 exampleService라는 이름으로 가져다 쓰겠다.
	
	@GetMapping("/ExampleView2")
	//매핑 : view페이지에서 보낸 신호를 받는 것. "/ExampleView"라고 온 신호가 들어오면 이하의 메서드를 실행하겠다.
	...
	// Model model : Model이라는 객체를 불러와서 model이라는 이름으로 가져다 쓰겠다.
	// **JSP(JavaServerPage) : 웹페이지를 만드는 것이 HTML이다. 근데 그 HTML에 자바코드까지 섞어쓸 수 있게 해주는 것.
		...
		//  exampleService.getExampleDBList() : exampleService의 getExampleDBList() 메서드를 실행하겠다.
		// List<ExampleEntity> ExampleDBList : 메서드의 반환값은 ExampleEntity 객체들을 담은 List이며,
		...
		return "ExampleView";
		// 이 모든 과정들을 ExampleView 페이지에 반환한다.
	}
}
